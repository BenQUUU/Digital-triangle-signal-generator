...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
	void ustawStanWe1(void);
	void obliczSrednia(void);
...
//	  BSP_LCD_DisplayStringAt(200, 20, (uint8_t *)xTouchStr, LEFT_MODE);
	  ...
	  if((licznik % 10) == 0){
		  HAL_GPIO_TogglePin(LedH5_GPIO_Port, LedH5_Pin);
	  }
	  if((licznik % 100) == 0){
	  ...
	  if(stanWe1 != 2){
		  ...
		  if(tempX >= 60 && tempX <= 110 && tempY >= 250 && tempY <= 270){
			  stanWe1 = 1;
	  ...
	  if(tempX >= 390 && tempX <= 420 && tempY >= 250 && tempY <= 270){
		  czestotliwoscZadana = 10;
	  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LedH7_GPIO_Port, LedH7_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(OTG_FS_PowerSwitchOn_GPIO_Port, OTG_FS_PowerSwitchOn_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOI, ARDUINO_D7_Pin|LedH6_Pin|LCD_DISP_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOG, LedH5_Pin|ARDUINO_D2_Pin|EXT_RST_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : LedH7_Pin */
  GPIO_InitStruct.Pin = LedH7_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_PULLDOWN;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(LedH7_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pin : We1_Pin */
  GPIO_InitStruct.Pin = We1_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(We1_GPIO_Port, &GPIO_InitStruct);
...
/* USER CODE BEGIN 4 */
void ustawStanWe1(void){
	if(!HAL_GPIO_ReadPin(We1_GPIO_Port, We1_Pin)){
		stanWe1 = 2;
	}else{
		stanWe1 &= 0xFFFD;
	}
...
void obliczSrednia(void){
	...
	przesuniecie = ((suma / 4.0) / 4096.0) + 0.01;

	if(przesuniecie <= 0.05){
		przesuniecie = 0.05;
	}

	if(przesuniecie >= 0.95){
		przesuniecie = 0.95;
	}
}
